...
def print_header():
 ...
 |_|\_\_| |_|\___/ \___|_|\_\ .__/ \__, |
                            ...
                            |_|    |___/ 
"""+font.color('end')
	print font.color('red')+"Modified By Rishabh Sharma"
	print "Twitter: @blacknet22\n"
	print "Count feature added and 2lakh+ wordlist added\n"+font.color('end')
def print_header_scan():
	print '\nIp Address\tStatus\tType\tDomain Name\t\tCount'
	print '----------\t------\t----\t-----------\t\t-----'

subdomain_csv_list = []
def print_output(data,countnumb):
	if data['alias']:
		for alias in data['alias']:
			ip_alias = data['ipaddress'][0]
			row = ip_alias+'\t'+str(data['status'])+'\t'+'alias'+'\t'+str(alias)+'\t'+countnumb
			print (row)
			subdomain_csv_list.append(row.replace('\t', ','))
		for ip in data['ipaddress']:
			row = ip+'\t'+str(data['status'])+'\t'+'host'+'\t'+str(data['hostname'])+'\t'+countnumb
			print (row)
			subdomain_csv_list.append(row.replace('\t', ','))
	else:
		for ip in data['ipaddress']:
...
def main():
	...
	wordlistcount=str(wordlist_count)
	...
	scan for subdomain
	'''
	init('- scaning for subdomain...WordList_Count:'+font.color('red')+wordlistcount+font.color('end'), True)
			...
			if wildcard_json['enabled']:
				...
				if str(status_code) != '' and str(wildcard_code) != '' and str(status_code) == str(wildcard_code):
					...
					if content_length == '0' or str(content_length) == str(wildcard_content_length):
						...
						print_output(subdomain_resolve,test)
